...
crusader.takeTurn = (self) => {
  ...
  if (availableSignals.length > 0){
    ...
    if(code == message.ATTACK){
      newTarget = true;
      //self.log(availableSignals);
      targetLocations.push(message.decode_location(availableSignals[0].signal));
      currentPath = [];
  ...
  if(newTarget == true){
    ...
    //  
     // self.log(targetLocations[targetLocations.length-1]);
      //self.log("end");
      targetCounter++;
      self.signal(message.encode_attack(targetLocations[targetCounter]), Math.pow(self.getPassableMap().length, 2));
    }
    //self.log("here");
    var nexStep = movement.get_next_step_astar_turn([self.me.x,self.me.y],targetLocations[targetCounter],self.map,

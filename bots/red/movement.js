...
export function findClosestCastle(myLocationX,mylocationY,possibleCastleLocations,map,rob){
  var i = 0;
  var minLength = 4096 ;
  var length = 0;
  var closestIndex = -1;
...
export function getRandomPath(direction,openPaths){
...
export function getPossibleSteps(coor,map){
  var result = [[],[],[]];//[y][x]
  var i;
  var j;
  for(i = 0;i < 3;i++){
...
export function getDirection(coor1,coor2){
...
//only single step
export function getNextStep(currentLocation,destination,map){
...
export function findPathToCoordinate(origin,destination,map){
  var path = [];

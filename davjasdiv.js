...
castle.takeTurn = (self) => {
    ...
    if (self.step >= 3 && pilgrimCount !== 0 && self.karbonite >= 10 && self.fuel >= 50) {
      ...
      if((self.step <= 350+ enemy_castles.length  && self.step > 350)||
        ...
        self.signal(encode_attack([enemy_castles[toSend].x,enemy_castles[toSend].y]), Math.pow(Math.ceil(self.getPassableMap().length/ enemy_castles.length), 2));
        locSentCounter++;
      }
...
prophet.takeTurn = (self) => {
  ...
  var availableSignals = visible.filter(robot => (robot.unit == 0 || robot.unit == 1) && robot.signal >= 14);
  if (availableSignals.length > 0){
    //self.log(availableSignals);
    var code = availableSignals[0].signal &  15;
    //self.log("my code is: " +code);
    if(code == ATTACK){
      newTarget = true;
      //self.log(availableSignals);
      targetLocations.push(decode_location(availableSignals[0].signal));
      currentPath$2 = [];
  ...
  if(newTarget == true){
    ...
    //  
     // self.log(targetLocations[targetLocations.length-1]);
      //self.log("end");
      targetCounter++;
      self.signal(encode_attack(targetLocations[targetCounter]), Math.pow(self.getPassableMap().length, 2));
    }
    //self.log("here");
...
crusader.takeTurn = (self) => {
  ...
  if (availableSignals.length > 0){
    ...
    if(code == ATTACK){
      newTarget$1 = true;
      //self.log(availableSignals);
      targetLocations$1.push(decode_location(availableSignals[0].signal));
      currentPath$4 = [];
      //self.log(targetLocations);
      //self.log(message.decode_location(availableSignals[0].signal));
    }
  ...
  if(newTarget$1 == true){
    ...
    //  
     // self.log(targetLocations[targetLocations.length-1]);
      //self.log("end");
      targetCounter$1++;
      self.signal(encode_attack(targetLocations$1[targetCounter$1]), Math.pow(self.getPassableMap().length, 2));
    }
    //self.log("here");
